...
const App = {
    ...
    openPosition: async function() {
        ...
        const sAsset = document.getElementById("sAsset").value;
        const sAssetContract = this.meta[sAsset].options.address;
        const collateralAmount = (document.getElementById("deposit").value);
        const collateralRatio = document.getElementById("CR").value;
        console.log(sAsset, collateralAmount, collateralRatio);
        
        await this.approve('EUSD', this.meta['Mint'].options.address, (parseInt(collateralAmount * 10**8)).toString());
        await this.meta['Mint'].methods.openPosition((parseInt(collateralAmount * 10**8)).toString(), sAssetContract, collateralRatio).send({ from: this.accounts[0] });
    ...
    addLiquidity: async function() {
        ...
        const liquidity0 = parseInt(document.getElementById("liquidity0").value);
        const liquidity1 = parseInt(document.getElementById("liquidity1").value);
        const reserves = await this.meta['Swap'].methods.getReserves().call();
        console.log(reserves[0], reserves[1]);
        console.log(liquidity0, liquidity1);

        try {
            assert(reserves[1] > 0 && reserves[0] > 0);
        } catch (err) {
            this.setStatus("Cannot add liquidity before initializing the pool.");
            return;
        }
        
        await this.approve('sBNB', this.meta['Swap'].options.address, parseInt(liquidity0* 10**8).toString());
        await this.approve('sTSLA', this.meta['Swap'].options.address, parseInt(((reserves[1] * liquidity0) / reserves[0])* 10**8).toString());

        await this.meta['Swap'].methods.addLiquidity((liquidity0* 10**8).toString()).send({from: this.accounts[0]});
        
        // if (liquidity1 < ((reserves[1] * liquidity0) / reserves[0])) {
        //     this.setStatus("More sTSLA required."); 
        // } else {
        this.refreshBalance();
        this.setStatus("Transaction complete!");
        // }
    ...
    removeLiquidity: async function() {
        ...
        const withdrawShares = document.getElementById("shares").value;
        await this.meta['Swap'].methods.removeLiquidity(withdrawShares).send({from: this.accounts[0]});
    ...
    token0To1: async function() {
        ...
        const token0Amount = document.getElementById("swap0").value;
        await this.approve('sBNB', this.meta['Swap'].options.address, token0Amount*10**8);
        await this.meta['Swap'].methods.token0To1(token0Amount*10**8).send({from: this.accounts[0]});
    ...
    token1To0: async function() {
        ...
        const token1Amount = document.getElementById("swap1").value;
        await this.approve('sTSLA', this.meta['Swap'].options.address, token1Amount*10**8);
        await this.meta['Swap'].methods.token1To0(token1Amount*10**8).send({from: this.accounts[0]});
